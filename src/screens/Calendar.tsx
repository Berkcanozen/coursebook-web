  ...
  return (
    <>
      {state.children.length > 1 && (
        <div className="filterbar cal-filter" style={{ flexWrap: 'wrap' }}>
          ...
          {state.children.map((c) => (
            <button key={c.id} className={'f' + (childFilter === c.id ? ' on' : '')} onClick={() => setChildFilter(c.id)}>
              <span style={{ background: c.color, display: 'inline-block', width: 8, height: 8, borderRadius: '50%', marginRight: 6, verticalAlign: 'middle' }} />
              {c.name}
            </button>
          ))}
        </div>
